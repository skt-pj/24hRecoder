...
def replace_all(path: str, old: str, new: str, minimum: int = 1) -> None:
    ...
    if count < minimum:
        raise RuntimeError(f"missing patch anchors in {path}: expected {minimum}, got {count}")
    p.write_text(text.replace(old, new))


# Version follows the actual GitHub latest 0.7.14 / versionCode 1014.
replace_once(
    "app/build.gradle",
    "versionCode 1014\n        versionName '0.7.14'",
    "versionCode 1015\n        versionName '0.7.15'",
...
replace_once(
    "app/build.gradle",
    "        // Low-SNR accepted speech can be denoised with AndroidDeepFilterNet before Whisper without compacting time.\n",
    "        // Low-SNR accepted speech can be denoised with AndroidDeepFilterNet before ASR without compacting time.\n"
    "        // ASR/VAD/denoise/speaker paths are explicitly user-selectable; backend failure never auto-falls back.\n",
)

# The cross-compiled Vulkan backend needs a host GLSL compiler for embedded shaders.
replace_once(
    ".github/workflows/build-apk.yml",
    "      - name: Build unsigned debug APK\n        run: gradle --stacktrace :app:assembleDebug\n",
...
# Settings UI: pipeline selection appears above model selection.
replace_once(
    "app/src/main/java/com/sktpj/recorder24h/WhisperModelSettingsCard.kt",
...
# Main UI/startup uses selected route requirements rather than unconditional Whisper readiness.
replace_once(
    "app/src/main/java/com/sktpj/recorder24h/MainActivity.kt",
...
replace_all(
    ...
    "TranscriptionPipelineSettings.isSelectedPipelineReady(context, WhisperModelManager.selectedModelId(context))",
    minimum=2,
)
replace_once(
    "app/src/main/java/com/sktpj/recorder24h/MainActivity.kt",
...
# Recorder sends the same captured PCM to the selected streaming VAD path as well as the retained cheap gate.
replace_once(
    "app/src/main/java/com/sktpj/recorder24h/audio/AacSegmentRecorder.java",
...
# Selected DeepFilterNet is strict: true processing failure is an item failure, not raw-audio fallback.
replace_once(
    "app/src/main/java/com/sktpj/recorder24h/transcription/DeepFilterNetSpeechDenoiser.kt",
...
# whisper.cpp JNI: explicit Vulkan flag plus stateful streaming Silero.
replace_once(
...
stream_marker = 'extern "C" JNIEXPORT jstring JNICALL\nJava_com_sktpj_recorder24h_transcription_LocalWhisperEngine_nativeAnalyzeVadDetailed('
streaming_native = r'''extern "C" JNIEXPORT jboolean JNICALL
...
'''
replace_once("app/src/main/cpp/whisper_jni.cpp", stream_marker, streaming_native + stream_marker)
replace_once(
    "app/src/main/cpp/whisper_jni.cpp",
    "        jstring language,\n        jint threads) {",
    "        jstring language,\n        jint threads,\n        jboolean use_gpu) {",
...
# LocalWhisperEngine dispatches exactly the selected VAD/ASR/denoise path.
replace_once(
    "app/src/main/java/com/sktpj/recorder24h/transcription/LocalWhisperEngine.java",
...
# Model-comparison calls remain explicit CPU/candidate paths and are independent of normal selected ASR.
replace_once(
    "app/src/main/java/com/sktpj/recorder24h/transcription/LocalWhisperEngine.java",
...
# Scheduler only requires resources for the explicitly selected route.
...
old_ready = '''        if (!WhisperModelManager.isReady(context)) {
...
'''
new_ready = '''        TranscriptionPipelineSettings.Snapshot pipeline = TranscriptionPipelineSettings.snapshot(context);
...
'''
if scheduler_text.count(old_ready) != 2:
    raise RuntimeError(f"scheduler readiness anchor count {scheduler_text.count(old_ready)}")
scheduler.write_text(scheduler_text.replace(old_ready, new_ready))

# Background single runner captures settings once for each attempt.
replace_once(
...
# Foreground direct runner uses the same exact settings.
replace_once(
...
print("transcription backend integration patch applied")
